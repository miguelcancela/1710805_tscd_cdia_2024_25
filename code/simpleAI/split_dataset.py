...
def size_x():
    global x
    data = pd.read_csv("./datasets/"+final_name+".csv", sep=",",header=None)
    x=len(data)-2
def num_rows(x):
    global x_train
    global x_test
...
df_train.to_csv('./datasets/'+final_name+'_train.csv', header=None,index=None)
df_test.to_csv('./datasets/'+final_name+'_test.csv', header=None,index=None)
